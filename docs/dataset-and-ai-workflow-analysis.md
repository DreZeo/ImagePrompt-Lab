...
- **知识规则进入垂直切片**：在通用摄影、海报、UI、二次元、信息图规则之外，新增 food-photography、architecture-interior、fashion-styling、tech-product-render、game-UI 五组高频垂直规则
- **输出配置升级为模型适配层**：除中英文、MJ、SD 外，新增 DALL-E、Flux、国产中文模型三类 profile，让同一知识链可按目标模型调整 prompt 形态
...
| **知识规则扩到 16 条，但仍是第一波** | 已补 food-photography、architecture-interior、fashion-styling、tech-product-render、game-UI 五组规则，但更多垂直领域仍可继续扩展 |
| **意图映射扩到 19 个，但中文自然表达仍有长尾** | 已新增详情页、穿搭封面、数据看板、food、interior、tech render、game UI 等细分意图；后续仍可继续补更多口语化短语 |
| **输出配置扩到 7 个，但模型适配仍偏家族级** | 已新增 DALL-E、Flux、国产中文模型 profile；后续仍可继续下钻到更细的厂商/模型差异 |
...
1. **继续扩充垂直规则**：在现有五组新增规则之外，后续可继续补 food-styling 细分、家装/工装拆分、beauty/fashion 子领域、游戏活动页等
2. **继续补充结构化主轨道模板**：在现有 11 个场景模板基础上，继续扩展行业化和平台化场景
3. **继续丰富意图映射**：重点补更多中文口语化短语、平台黑话和组合任务表达，提升命中率
4. **细化模型适配 profile**：当前已到家族级，后续可继续细分不同国产模型或 OpenAI/Flux 子版本的偏好
